...
def calculate_mtf_width(profile, peak, trough, all_peaks=None, all_troughs=None):
    """
    Calculate MTF width using the trough value as the tangent line.
    Measures the full width of the entire 3-bar element.
    """
    # Use the trough value as the tangent line height
    tangent_height = profile[trough]
    
    # Create high-resolution interpolation of the profile
    ...
    # If we have at least 2 crossings, we can calculate the width
    if len(crossings) >= 2:
        # For accurate MTF, we need to measure the full width of the entire element
        # This means finding the outermost crossings of the tangent line
        
        # Simply use the first and last crossing points
        # This captures the full width of the entire 3-bar element
...
def process_element(roi, roi_display, group, element_num, px, py):
    ...
    # Calculate MTF width using trough as tangent line, considering all 3 bars
    width, mtf_value, tangent_height = calculate_mtf_width(profile, peak, trough, peaks, troughs)
