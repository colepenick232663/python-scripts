...
def extract_psf(image, point, psf_size=15):
    """Extract the PSF around the brightest point."""
    x, y = point
    half_size = psf_size // 2
    psf = image[y - half_size:y + half_size + 1, x - half_size:x + half_size + 1]
    return psf
...
def apply_deconvolution(image, mtf, noise_var=0.01):
    """Apply Wiener deconvolution using the MTF."""
    mtf_shifted = fftshift(mtf)
    H = mtf_shifted
    H_conj = np.conj(H)
    H_mag_sq = np.abs(H) ** 2
    wiener_filter = H_conj / (H_mag_sq + noise_var / np.abs(image) ** 2)
